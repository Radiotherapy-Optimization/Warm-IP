"""
Benchmark of QP solvers on the Maros-Meszaros convex QP test set.

Loads instances from MM_data/ through data_loader. They are stored in the
...
term on mixed problems). Results go to results/MM_solver_benchmark.csv;
...
# instance number (1, 10, 17 -> MM_001..., MM_010..., MM_017...), a full name
# ("MM_001_AUG2D"), or the original short name ("AUG2D", "CVXQP1_L").
# ---------------------------------------------------------------------------
PROBLEMS = "all"
...
# Per-instance convergence and time-comparison plots. With the full set of
# 137 problems this writes several hundred PNGs, so it is off by default.
MAKE_PLOTS = False
...
#ISOLATE_SKIP = ("MOSEK",)
ISOLATE_SKIP = ()
SOLVER_TIMEOUT = 360      # per-solver limit, applied to EVERY isolated solve

# All program outputs (plots, CSV) are written here
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'results')
os.makedirs(OUTPUT_DIR, exist_ok=True)
CSV_PATH = os.path.join(OUTPUT_DIR, 'MM_solver_benchmark.csv')
...
WARM_IP_config = {
    ...
    'ruiz': True,        # Ruiz equilibration preprocessing (OSQP algorithm)
}
...
def benchmark_qp_solvers(data_dir=None):
    all_results = []
    names = select_instances(PROBLEMS, data_dir, prefix="MM_")
...
if __name__ == "__main__":
    ...
    print("Benchmark complete. Results saved to results/MM_solver_benchmark.csv.")
